...
*Change the date of update of intranet;
%let ThisYear=2017;
*Import file of last time;
%let LastTime=2014;
...
length MACRO1 $100.;
MACRO1="NEWSURV"; 
DATEMODIF="02NOV2017"d;
HMODIF=.;
SIZE=308;
NAME="newsurv";
use="Y";

format DATEMODIF ddmmyy10.  HMODIF TIME20.3; 
run;
...
DATA EORTC_LIST;
	...
	** JRA, Feb 2013;
	...
	if author1="JR" then author1="JRO";
	if author2="JR" then author2="JRO";
	if author3="JR" then author3="JRO";
...
DATA testMOD;
	...
	"GBLABELS" "SUMMARY" "SUMMARYB2" "YEARLYFP" "CUMULMA" "FLEMING" "GALBRAIT" "RISKDIFF" 
		...
		if MACRO1="PREDICT_EVENTS" then do;
			ADD='JRA';
			origdate=DATEMODIF;
			desc_sh="Makes prediction of the number of events";
			type="A";
			notes="Makes prediction of the number of events to be received in the future in the presence of diminishing/evolving hazard with time since randomization";
		end;
	...
	if MACRO1 in ("AE_TABLE" "AE_TABLE2" "ALLFIXED2"	"ALLFIXEDB2"	"CLEAN_TABLE"	"FOREST2"	"FORESTB2"	
	...
	"PRED_FACT_SUMMARY" "PRED_FACT_FOREST" "PREDICT_EVENTS" "TABLES3" "LAB_TOX") then valid='Y'; 
	...
	if MACRO1 in ("AE_TABLE" "AE_TABLE2" "AUTO_REPORT" "NEWSURV") then add="GIS";
	...
	if MACRO1 in (	 "LAB_TOX"
					  ...
					  "LABLIST_GEN"
					  "COMPUTE_CTC_GRADE"
					  "DETECT_PROBLEMS" "WATERFALL") 
	then do;
			origdate=mod;
		end;

		if MACRO1="TABLES3" then do;
			origdate='17APR2015'd;
			type="R";
			desc_ln="This version let the user to compute the percentages including or excluding all missing values in the TOTAL";
			desc_sh="Tables for study reports (% on column, row or Grand total)";
			BasedOn="TABLES2.sas";
			 ADD="LC ";ADD1="KM "; ADD2="JRA"; 
		end;

		if MACRO1="AE_TABLE" then do;
			origdate='16SEP2016'd;
			type="R";
			desc_ln="This version presents the AEs coded in CTCv4. Build for standard CRF of newly designed studies.";
			desc_sh="Tables and graphs for adverse events";
			BasedOn="";
			ADD="GIS";
		end;
		if MACRO1="AE_TABLE2" then do;
			origdate='22NOV2017'd;
			type="R";
			desc_ln="This version 2 presents the AEs coded in CTC4 and CTC3. Build for standard CRF of newly designed studies.";
			desc_sh="Tables and graphs for adverse events";
			BasedOn="";
			ADD="GIS";
		end;
		if MACRO1="AUTO_REPORT" then do;
			origdate='22NOV2017'd;
			type="R";
			desc_ln="This code sends the output from SAS to the template of clinical report";
			desc_sh="Automation of report";
			BasedOn="";
			ADD="GIS";
		end;
		if MACRO1="NEWSURV" then do;
			origdate='02NOV2017'd;
			type="G";
			desc_ln="This macro creates KMPlot without using GraphLib";
			desc_sh="Survival curve";
			BasedOn="";
			ADD="GIS";
		end;
		if MACRO1="EUDRACT_REPORTING" then do;
			origdate='15SEP2017'd;
			type="R";
			desc_ln="This macro report AEs in a excel file ready to be converted into xml and imported on the EUDRACT website";
			desc_sh="EUDRACT AEs";
			BasedOn="";
			ADD="GIS";
		end;
...
DATA ML.macrolist&ThisYear.;
	length desc_sh $60. Documentation $280.;
	...
	"PHREGTEMPLATE" "PRED_FACT"	"PRED_FACT_SUMMARY" "PRED_FACT_FOREST" "PREDICT_EVENTS" "TABLES3" "AE_TABLE" "AE_TABLE2"
	"AUTO_REPORT" "NEWSURV"
...
data _NULL_;
set ML.macrolist&ThisYear.;
if MACRO1="QLSCORE" then call symput('modscore',compress(datemodif));
if MACRO1="TABLES3" then call symput('modtables3',compress(datemodif));
run;

*%put modscore &modscore. modtables3 &modtables3;
...
data Processed;
	...
	if documentation="link" then do;
		** META ANALYSIS**;
		...
		documentation="<A href='\\elvis\Pccommon\UNIT\Stat\7. SAS and R\SAS Macros Documentation\META ANALYSIS\ALLFIXED2 Documentation 1.3.doc'>link</A>  <A href='\\elvis\Pccommon\UNIT\Stat\7. SAS and R\SAS Macros Documentation\META ANALYSIS\ALLFIXED2 - new version.March 2015.pps'>new version</A>";
		...
		** TABLES2**;
		...
		if macro1="TABLES3" then
		documentation="<A href='\\elvis\Pccommon\UNIT\Stat\7. SAS and R\SAS Macros Documentation\TABLE\TABLES3  Documentation 1.0.doc'>link1</A>  <A href='\\elvis\Pccommon\UNIT\Stat\7. SAS and R\SAS Macros Documentation\TABLE\TABLES3 macro.pptx'>link2</A>";
		...
		** SURVIVAL ANALYSIS**;
	    ...
	    if macro1="PREDICT_EVENTS" then
		documentation="<A href='\\elvis\Pccommon\UNIT\Stat\7. SAS and R\SAS Macros Documentation\SURVIVAL ANALYSIS\PREDICT_EVENTS Documentation 1.0.docx'>link1</A> <A href='\\elvis\Pccommon\UNIT\Stat\7. SAS and R\SAS Macros Documentation\SURVIVAL ANALYSIS\PREDICT_EVENTS.pptx'>link2</A>";
		...
		** ADVERSE EVENTS **;
		if macro1="AE_TABLE" then
		documentation="<A href='J:\UNIT\Stat\7. SAS and R\SAS Macros Documentation\ADVERSE EVENTS TABLES AND GRAPHS\AE_table_documentation(V1.0).docx'>link1</A> <A href='J:\UNIT\Stat\7. SAS and R\SAS Macros Documentation\ADVERSE EVENTS TABLES AND GRAPHS\AE_Table.pptx'>link2</A>";

		** ADVERSE EVENTS VERSION 2**;
		if macro1="AE_TABLE2" then
		documentation="<A href='J:\UNIT\Stat\7. SAS and R\SAS Macros Documentation\ADVERSE EVENTS TABLES AND GRAPHS\AE_Table2_Macro documentation.pdf'>link1</A>";

		** NEWSURV**;
		if macro1="NEWSURV" then
		documentation="<A href='J:\UNIT\Stat\6. Statsclub\1. Presentations\SAS and programming\GIS_20170926_KMmacro_newsurv.pptx'>link1</A> <A href='J:\UNIT\Stat\7. SAS and R\SAS Macros Documentation\SURVIVAL ANALYSIS\NEWSURV\NEWSURV- List of parameters.pdf'>link2</A>";

		** AUTO_REPORT**;
		if macro1="AUTO_REPORT" then
		documentation="<A href='J:\UNIT\Stat\6. Statsclub\1. Presentations\SAS and programming\GIS_statsclub_20171120_Automation_report.pptx'>link1</A>";
		end;
	...
	IF NAME="SCORES" THEN DO;
		NAME2="QOL_SCORES";
		current='Y';
		NAMETYPE="<DIV>QOL_SCORES<BR>(Quality of Life)</div>";
		mod=. ; *mod=&modscore.;

	END;
	IF NAME="QOL_SCORES" and Purpose="" THEN delete;
...
options pageno=1;
ods html file="K:\SAS\EORTC macros\Intranet Lists\Output\CurrentMacros.html" style=mystyle;
...
proc print data=List label noobs
           style (header) = header [just=l]
           style (data) = data [just=l];
var NameType Purpose Authors OrigDate Mod ;
run;
title1;
ods html close;
