...
*                   Add of RowPercent parameter which let you have row percentage instead 
*                   of column percentage for categorical variable. (JRA)
...
*    ODS       = Y if ODS output required (default is N)
...
*    RowPercent= Y if you want to have row percentage instead of column percentage 
*                for categorical variable. (default is N)
*                In this case, TABLES2 has the same fonctionnement as TABLESR2 macro.
*                But with this option, it is not possible to have TOTAL column.
...
*    Pct       = N if you want to delete display of percentages for categorical variable (default is Y)
*               
*************************************************************************************
*
* NOTES
...
%macro Tables2(data,var,by,where,title,num,total,header=Y,ODS=N,ODSFile=,Mean=N,Both=N,Quantiles=N,RowPercent=N,ordfreq=N,reduce=N,Pct=Y) ;
...
%let varlist=&var;
%let i=1 ;
%IF &RowPercent.=Y %then %let TOTAL=N;
 ...
 ***********************************************;
   %IF (&num^=Y) %THEN %DO ;
      ...
      proc freq data=&data noprint ;
      tables &var*&by / norow nopercent missing out=__tab sparse outpct;
      where &where ;
      run;
	  ...
	  %else %do;
		 ...
		 %end;
		 %else %do;%sort(data=__tab,var=&var &by);%end;
	  %end;
     ...
     %IF &ODS.=Y %THEN %DO;
         ...
         run;
  		  ...
  		  ********* Add for validation test (JR - 12-07-2006) **;
				data __ttab&i.;
				set __tab;
		 		run;
		  ******************************************************;
       ...
       data __Output&i.;
         ...
         do i=1 to %eval(&Ncat) ;
           set __tab end=eof ;
           cnt[i]=count ;
		   %if &RowPercent.=N %then xx=round(pct_col,0.1) ;;
 		   %if &RowPercent.=Y %then xx=round(pct_row,0.1) ;;
          if (xx=floor(xx)) then yy=compress(put(xx,8.))||'.0' ;
           else yy=compress(put(xx,8.1)) ;
           if (length(compress(yy))<4) then yy=' '||yy ;
           pct[i]=compress('('||yy||')') ;
           cntpct[i] = put(cnt[i],4.)||' '||trim(left(pct[i]));
		   %if &pct.=N %then  cntpct[i] = put(cnt[i],4.);;
         end ;
         length VarLabel $200.;
         call label(&var.,VarLabel);
 	  ...
 	  %ELSE %DO;
         ...
         data _NULL_ ;
          ...
          %end ;
              ...
              call label(&var,lab) ;
              call label(&by,top) ;
              rtop=floor(length(trim(left(top)))/2) ;
              ...
              %ELSE %IF &VarFmtType.=%STR(SASC) %THEN %DO;
                 ...
                 put @10 VarValue @;
              %END;
              %ELSE %STR(put @10 &var &VarFmtName.. @;);
              put %do j=1 %to &Ncat ;
         ...
         %if (&first=Y) %then %do ;
			...
			**JRA, Oct2007, Add of PCT parameter;
	         ...
	         %do j=1 %to &Ncat ;
	              ...
	              %if &PCT=N %then %do; @(&dist+5+&j*15) "N"   %end;
	              %else %do;@(&dist+3+&j*15) "N"  @(&dist+9+&j*15) "(%)" %end;
	         %end ;
	         ;
              put @5 &last*'_' /;
                RETURN;
 ...
 *******************************************;
   ...
   %IF (&num=Y) %THEN %DO ;
      ...
      data _NULL_ (keep=lab);
      set __sorted end=lastobs;
      length lab $200.;
      call label(&var,lab) ;
      if lastobs then call symput('LAB',trim(left(lab))) ;
      run;
...
**************************************************;
	...
	*	%gmax(data=__out2,var=r,outdata=__t,outvar=max);
		...
		*%gmax(data=__out3,var=r,outdata=__t1,outvar=max);
		**JRA, 11JUN2012, Beacause of SAS 9.3 update, modification of GMAX by proc means to remove unexpected titles
		"PROC UNIVARIATE" in case of use of TABLES 2 with ODS RTF with BODYTITLE in SAS 9.3;
		 ...
		 *Sept2007, JRA, Change of the size of columns tables initialization;
		  ...
		  **********************************************;
	      ...
	      define table TablesTemplate;
	         ...
	         column FirstCol %DO j = 1 %TO &NCat.; ResultCol&j. %END; ;
			 mvar Title;
	         define TableTitle;
	            text Title;
	         	style = header {                                                     
            ...
            file print
                 ods=(template='TablesTemplate'
                      columns=(FirstCol=VarValue 
							...
							%if &PCT.=N %then (dynamic=(ColHeader='N' ));
  							%else 	(dynamic=(ColHeader='N (%)' ));
                             %END; ));
            put _ods_;
            run;
...
************************************** ********;
		...
		**JRA, 11JUN2012, Beacause of SAS 9.3 update, modification of GMAX by proc means to remove unexpected titles
		 ...
		 ** Determination of maximum size for BYVAR column *;
		 ...
		 data __outB21;
		 set __outB2 	%IF &form. ne 8 AND &form. ne BEST %then %do;
			__s(in=a)
			%end;
			;
			%IF &form. ne 8 AND &form. ne BEST %then %do;
	...
	*	%gmax(data=__outB21,var=lt,outdata=__t,outvar=max );
	...
	*	%gmax(data=__outB21,var=lt&Ncat,outdata=__t2,outvar=max);
...
%IF &form. ne 8 AND &form. ne BEST %then %do;
	...
	proc format lib=work cntlout=__format2;
	run;
