...
%MACRO RECOGNITION (data=,varlist=,keeplist=,period=);
	...
	** Recuperation of all variable names and labels from INPUT dataset;
	...
	data _contentlab;
		...
		if name=:'TXLAB' or name=: 'TXCOM' then delete;
		if name=:'LAB' then delete;
		if name="&PERIOD." then delete;
		if name=:'VISIT' or name=: 'CYCLE' or name=:'DT' then delete;
		if name=:'AEREF' then delete;
		if name=:'NY' then delete;
		...
		*Identify 'related AE';
			*Possibility number 1;
			...
			if UPCASE(name) = 'PTINR' then flag_r=0; *not related;
			if UPCASE(name) = 'GFR' then flag_r=0; *not related;
			*Possibility number 2;
			if INDEX (UPCASE(LABEL), ' AE ')>0 then flag_r=1;
		...
		*Identify SAE;
			...
			*Possibility number 2;
			if INDEX (UPCASE(LABEL), ' SAE ')>0 then flag_s=1;
	...
	** Creation of several macro variables (CODEi, LLNi, ULNi, SAEi, RELi, ATi) from the LAB dictionnary;
	data _null_;
		set _Lab_dictionnary END=FINAL;
		N+1;
	...
	DATA _null_;
	set _check(where=(checkdico = 0)) end=eof;
	N+1;	
	...
	%IF &INITVARLIST. NE 0 %THEN %DO;
		...
		** if checklist2 =1 then it means that one of all variable names from
		...
		DATA _CHECKVARLIST;
			...
			* check if the variables in VARLIST exist in the LAB dictionnary;
			%IF %length(&CONTENTINITVARLIST.) ne 0 %then %do;
					%do j=1 %to &Nvarlistb;
						%do i=1 %to &nb;
							_varlistC&j.=0;
							varlist&j.="&&w&j";
							if varlist&j. =: "&&code&i"  then _varlistC&j.=1;
						%end;
						**GIS, 2017: if _varlistC&j.=1 it means that this variable listed in VARLIST exists in the dictionnary
						 ==> warning**;
						if _varlistC&j.=1 then do;
							%put WARNING: ***-----------------------------------------------------------***;
							%put WARNING: The variable &&w&j in VARLIST seems to exist in the dictionnary. ;
							%put WARNING: Please check 												       ;
							%put WARNING: ***-----------------------------------------------------------***; 
						end;
					%end;
			%END;
   ...
   %let check=0;
   data _null_;
		set _check END=FINAL;
		N+1;
		IF FINAL THEN CALL SYMPUT('check',COMPRESS(N));
   run;

   proc means data =_check noprint;	
      var checklist ;
	  output out=_checklist sum=sum;
   run;
	...
	** Produce checking listing; 
	...
	proc print data=_check noobs label ;
   ...
   %end;
	...
	%sort(data=_check2,var=NAME);

	data INPUT_VAR;
		merge INPUT_VAR(in=a) _check2(keep=NAME checklist ORIG);
		by NAME;
		if a;
	...
	proc datasets nolist;
		delete  _AT 
		 _CONTENTLAB 
		 _LAB_DICTIONNARY
		 _CHECK  _CHECK2 _CHECKLIST _CHECKVARLIST
		 _LL 
